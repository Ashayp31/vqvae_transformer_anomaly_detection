    ...
    if config["rank"] == 0:
        train_handlers += [
            VQVAELoggingHandler(
                ...
                log_3d=config["save_3d_images"] if config["data_spatial_dimension"] == 3 else None,
                log_2d=config["save_2d_images"] if config["data_spatial_dimension"] == 3 else None,
                log_image=True if config["data_spatial_dimension"] == 2 and  config["save_2d_images"] is not None else False,
            ),
...
def inference(config: dict) -> None:
    ...
    if config["mode"] == VQVAEModes.EXTRACTING.value:
        if config["data_spatial_dimension"] == 2:
            batch_transform = lambda batch : {'filename_or_obj':  batch["MRI_meta_dict"]["filename_or_obj"]}
        else:
        ...
        SegmentationSaver(
            ...
            batch_transform=get_batch_transform(
                ...
                is_nii_based=True,
                filename_or_objs_only=True if config["data_spatial_dimension"] == 2 else False,
                mode=config["mode"],
            ),
...
def run(
    ...
    save_2d_images: tuple = ("axial", "coronal"),
    save_3d_images: tuple = None,
    data_spatial_dimension: int = 2,

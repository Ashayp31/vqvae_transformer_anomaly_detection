...
class Quantizer_impl(nn.Module):
    def __init__(self, n_embed, embed_dim, eps, use_coordconv):
        super().__init__()
    ...
    def embed(self, embedding_indices: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_coordconv:
            quantized_st = torch.cat((quantized_st, quantized_st[:,0:3,:,:,:]), dim=1)
...
class Quantizer(nn.Module):
    def __init__(self, n_embed, embed_dim, commitment_cost=0.25, decay=0.99, eps=1e-5, use_coordconv=False):
        super().__init__()
        self.impl = Quantizer_impl(n_embed, embed_dim, eps, use_coordconv)
...
class BaselineVQVAE(VQVAEBase, nn.Module):
    def __init__(
        ...
        self.apply_coordConv = apply_coordConv
    ...
    def construct_quantizer(self) -> nn.ModuleList:
        quantizer = Quantizer(
            ...
            decay=self.vq_decay,
            use_coordconv=self.apply_coordConv
        )
        return nn.ModuleList([quantizer])

    def construct_decoder(self) -> nn.ModuleList:

        modules = [nn.Conv3d(self.embed_dim + 3 if self.apply_coordConv else self.embed_dim,
    ...
    def add_coordconv(self, imgs, crops):
        spatial_dims = (0,1,2)
        spatial_size = imgs.shape[2:]
        crops = crops.cpu().numpy()
        for x in range(imgs.shape[0]):
            img=imgs[x]

            coord_channels = np.array(
                np.meshgrid(*tuple(np.linspace(crops[x][k*2], crops[x][k*2+1], spatial_size[k]) for k in range(len(spatial_size))),
                            indexing="ij"))
            coord_channels = torch.as_tensor(coord_channels, device=imgs.device, dtype=imgs.dtype)
            coord_channels = coord_channels[list(spatial_dims)]
            imgs[x][-3:] = coord_channels
        return imgs
        
    def decode(self, quantizations: List[torch.Tensor], coord_vals: torch.Tensor = None) -> torch.Tensor:
        if self.apply_coordConv:
            quantization = self.add_coordconv(quantizations[0], coord_vals)
        else:
            quantization = quantizations[0]
        x = self.decoder[0](quantization)
        return x
    ...
    def decode_samples(self, embedding_indices: List[torch.Tensor], coords: torch.Tensor) -> torch.Tensor:
        ...
        samples_images = self.decode([samples_codes], coords)
    ...
    def forward(self, images: List[torch.Tensor]) -> Dict[str, List[torch.Tensor]]:
        ...
        reconstruction = self.decode(quantizations, coords)

...
from src.utils.vqvae_transformer import (
    get_data_flow,
    prepare_batch,
)
...
def inference(config: dict):
    ...
    if config["infer_mode"] == "anomaly_detection":
        ...
        #thresholds = [0.0001,0.00025,0.0005]
        # thresholds = [config["threshold"]]
    else:
        # thresholds = [config["threshold"]]
        thresholds = [0.001,0.0025,0.005]
    ...
    for thresh in thresholds:
        ...
        engine = SupervisedEvaluator(
            ...
            inferer=TransformerConditionalInferer(device=device,
                                                  ...
                                                  clip_encoding= True if "performer" in config["transformer_network"] else False)
            if config["infer_mode"] == "anomaly_detection"
        ...
        if config["infer_mode"] == "anomaly_detection":
            ...
            if config["token_masking"]:
                token_masking_add_on_name = "_token_masked"
            else:
                token_masking_add_on_name = ""

            name_addition = "combined_th_" + str(config["threshold"]).replace(".", "_") + "_npd_" + \
                                str(config["num_passes_dropout"]) + "_nps_" + str(
                    config["num_passes_sampling"]) + condition_add_name + token_masking_add_on_name
            # SegmentationSaver(
            #     output_dir=config["outputs_directory"],
...
def run(
        ...
        num_passes_dropout: int = 4,
        num_passes_sampling: int = 60,
    ...
    Args:
        ...
        vqvae_network_checkpoint (str): Path to the checkpoint of the saved vqvae model state to use

...
from src.utils.constants import TransformerModes, TransformerConditioningType
...
def training(config: dict):
    ...
    if config["vqvae_checkpoint"]:
        vqvae_network = get_vqvae_network(
            config={
                ...
                "dropout_penultimate": config["vqvae_net_dropout_penultimate"],
                "dropout_enc": config["vqvae_net_dropout_enc"],
                "dropout_dec": config["vqvae_net_dropout_dec"],
        ...
        if config["vqvae_conditioning_checkpoint"]:
            vqvae_conditioning_network = get_vqvae_network(
                config={
                    "network": config["vqvae_conditioning_network"],
                    "use_subpixel_conv": config["vqvae_net_use_subpixel_conv_cond"],
                    "use_slim_residual": config["vqvae_net_use_slim_residual_cond"],
                    "no_levels": config["vqvae_net_no_levels_cond"],
                    "downsample_parameters": config["vqvae_net_downsample_parameters_cond"],
                    "upsample_parameters": config["vqvae_net_upsample_parameters_cond"],
                    "no_res_layers": config["vqvae_net_no_res_layers_cond"],
                    "no_channels": config["vqvae_net_no_channels_cond"],
                    "codebook_type": config["vqvae_net_codebook_type_cond"],
                    "num_embeddings": config["vqvae_net_num_embeddings_cond"],
                    "embedding_dim": config["vqvae_net_embedding_dim_cond"],
                    "embedding_init": config["vqvae_net_embedding_init_cond"],
                    "commitment_cost": config["vqvae_net_commitment_cost_cond"],
                    "decay": config["vqvae_net_decay_cond"],
                    "norm": config["vqvae_net_norm_cond"],
                    "dropout_penultimate": config["vqvae_net_dropout_penultimate_cond"],
                    "dropout_enc": config["vqvae_net_dropout_enc_cond"],
                    "dropout_dec": config["vqvae_net_dropout_dec_cond"],
                    "act": config["vqvae_net_act_cond"],
                    "output_act": config["vqvae_net_output_act_cond"],
                }
            )
            vqvae_conditioning_network.to(device)
            Checkpoint.load_objects(
                to_load={"network": vqvae_conditioning_network},
                checkpoint=torch.load(config["vqvae_conditioning_checkpoint"], map_location=device),
                strict=True,
            )
        else:
            vqvae_conditioning_network = None
    ...
    config["vqvae_conditioning_network"] = vqvae_conditioning_network

    if config["vqvae_checkpoint"]:
        print("here for getting spatial shape")
        config["spatial_shape"] = (
            config["vqvae_network"]
            .index_quantize(next(iter(training_loader))["quantization"].to(device))[
    ...
    key_metric = {
        f"Metric-CE-Prediction": CE(
            output_transform=lambda network_output: (
                ...
                network_output[CommonKeys.LABEL][0],
            )
        )
    }
    ...
    trainer = SupervisedTrainer(
        ...
        prepare_batch=lambda batch, pb_device, non_blocking: prepare_batch(
            ...
            spatial_conditioning=config["spatial_encoding"],
            masked_tokens=config["token_masking"],
            vocab_size_enc=config["vocab_size_enc"],
            vqvae_cond_network=config["vqvae_conditioning_network"],
            vqvae_net_level=config["vqvae_net_level"],
            vqvae_augmentations=config["vqvae_augmentations"],
            use_vqvae_aug_conditionings=config["use_vqvae_aug_conditionings"],
            vqvae_augmentations_conditionings_key="quantization_trace_dict",
            ...
            rot90_axes=config["rot90_axes"],
            transformation_order=config["transformation_order"],
            spatial_buckets=config["spatial_conditioning_buckets"],
            device=pb_device,
...
def inference(config: dict):
        ...
        if config["vqvae_conditioning_checkpoint"]:
            vqvae_conditioning_network = get_vqvae_network(
                config={
                    ...
                    "decay": config["vqvae_net_decay_cond"],
                    "norm": config["vqvae_net_norm_cond"],
                    "dropout_at_end": config["vqvae_net_dropout_penultimate_cond"],
                    "dropout_enc": config["vqvae_net_dropout_enc_cond"],
                    "dropout_dec": config["vqvae_net_dropout_dec_cond"],
                    "act": config["vqvae_net_act_cond"],
                    "output_act": config["vqvae_net_output_act_cond"],
    ...
    if config["vqvae_checkpoint"]:
        config["spatial_shape"] = (
            config["vqvae_network"]
            .index_quantize(next(iter(evaluation_loader))["quantization"].to(device))[
                config["vqvae_net_level"]
            ]
            .shape[1:]
        )
    else:
        config["spatial_shape"] = next(iter(evaluation_loader))["quantization"].shape[
            1:
        ]
    ...
    # spatial_encoding_path: str = None,
    # max_seq_len: int = None,
    # fixed_ordering: bool = True,
    # spatial_conditioning_buckets: int = 8000,
...
def run(
    ...
    vqvae_net_no_res_layers: int = 3,
    vqvae_net_no_channels: int = 256,
    ...
    vqvae_net_no_res_layers_cond: int = 3,
    vqvae_net_no_channels_cond: int = 256,
    ...
    Args:    
        ...
        encoding_conditioning_path (str): Path towards a csv/tsv file that has a 'subject' column in which the file names from
            ...
            that represent the inputs of the conditioning via cross attention.
        ...
        vqvae_conditioning_checkpoint (str): Path to the vqvae checkpoint to be loaded and used to generated the augmented 
            representations of the conditioning data. Defaults to None.
                
        use_vqvae_aug_conditionings (bool): Whether or not the transformer is conditioned on the VQ-VAE augmentations. 
            Defaults to False.
        ...
        vqvae_network_cond (str): What vqvae network to use. Defaults to 'baseline_vqvae'.

        vqvae_net_level_cond (int): Which level of the VQ-VAE the transformer will be trained on. Defaults to 0.

        vqvae_net_use_subpixel_conv_cond (bool): Whether or not to use SubPixelConvolution as the last transpose convolution 
            in the network. Defaults to True.

        vqvae_net_use_slim_residual_cond (bool): Whether or not to have the kernel of the last convolution in each residual 
            unit be equal to 1. Default to True

        vqvae_net_no_levels_cond (int): How many levels the VQVAE has. Defaults to 3.

        vqvae_net_downsample_parameters_cond (Tuple[Tuple[int,int,int,int],...]): A Tuple of Tuples for defining the 
            downsampling convolutions. Each Tuple should hold the following information kernel_size (int), stride (int),
            padding (int), dilation(int). Defaults to ((4,2,1,1),(4,2,1,1),(4,2,1,1)).

        vqvae_net_upsample_parameters_cond (Tuple[Tuple[int,int,int,int,int],...]): A Tuple of Tuples for defining the 
            upsampling convolutions. Each Tuple should hold the following information kernel_size (int), stride (int),
            padding (int), output_padding (int), dilation(int). Defaults to ((4,2,1,0,1),(4,2,1,0,1),(4,2,1,0,1)).

        vqvae_net_no_res_layers_cond (int): How many residual layers we use per level. Defaults to 1.

        vqvae_net_no_channels_cond (int): How many channels the deepest level has. Defaults to 128.

        vqvae_net_codebook_type_cond (ema): What codebook type to use. Defaults to 'ema'.

        vqvae_net_num_embeddings_cond (Tuple[int,...]): How many codebook elements (atomic elements/tokens) each of the 
            embedding spaces has. Defaults to (32, ).

        vqvae_net_embedding_dim_cond (Tuple[int,...]): The channel dimension size of the elements of each codebook.
            Defaults to (64, ).

        vqvae_net_embedding_init_cond (Tuple[str,...]): The initialization used for the codebook elements. Can be either 
            'normal' or 'kaiming_uniform'. Defaults to ('normal',).

        vqvae_net_commitment_cost_cond (Tuple[float,...]): The commitment cost factor that will be used to scale the EMA 
            latent loss. Defaults to (0.25, ).

        vqvae_net_decay_cond (Tuple[float, ...]): The decay factor that will be used to update the embedding space when EMA
            Quantization update is being used. Defaults to (0.99, ).
        
        vqvae_net_norm_cond (str): Which normalization technique the network will use. Defaults to None.

        vqvae_net_dropout_cond (float): The amount of dropout use in the network. Defaults to 0.1

        vqvae_net_act_cond (str): Which activation function the network uses. Defaults to 'RELU'.

        vqvae_net_output_act_cond (str): Which activation function should the output be passed through. Defaults to None.
        ...
        vocab_size_enc (int): The size of the vocabulary for the encoded conditioning data. It must be the same values 
        as the "num_embeddings" argument used during the vqvae training. Defaults to 32.
    ...
    if config.get("spatial_encoding_path", None):
        config["spatial_encoding"] = True
    else:
        config["spatial_encoding"] = False

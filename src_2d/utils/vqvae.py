...
def get_transformations(
    ...
    if mode == VQVAEModes.DECODING.value:
        ...
        transform = [
            ...
            AsChannelFirstd(keys=[key])
        ]

        if data_num_channels == 1:
            transform += [AddChanneld(keys=["MRI"])]
        ...
        #prob 0.6
            augmentations += [
                RandAdjustContrastd(keys=["MRI"], prob=0, gamma=(0.975, 1.025)),
                RandShiftIntensityd(keys=["MRI"], prob=0, offsets=(0.0, 0.025)),
                RandGaussianNoised(keys=["MRI"], prob=0, mean=0.0, std=0.025)]

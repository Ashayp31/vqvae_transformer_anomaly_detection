...
class BaurLoss(_Loss):
    ...
    def __init__(
        ...
        self.dx = nn.ConstantPad2d((1, -1, 0, 0), 0)
        self.dy = nn.ConstantPad2d((0, 0, 1, -1), 0)
    ...
    def forward(
        ...
        gdl_loss = (
            getattr(
                torch.abs(
                    torch.abs((self.dx(y) - y))[:, :, 1:-1, 1:-1]
                    - torch.abs((self.dx(y_pred) - y_pred))[:, :, 1:-1, 1:-1]
                )
                + torch.abs(
                    torch.abs((self.dy(y) - y))[:, :, 1:-1, 1:-1]
                    - torch.abs((self.dy(y_pred) - y_pred))[:, :, 1:-1, 1:-1]
                ),
                self.reduction,
            )()
            * self.gdl_factor

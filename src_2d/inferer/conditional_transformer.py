...
class TransformerConditionalInferer(Inferer):
    ...
    def __call__(self, inputs: torch.Tensor, network: TransformerBase, *args: Any, **kwargs: Any):
        ...
        """
        if len(inputs) == 4:
            input_pet, input_ct, conditioning, token_mask = inputs 
        else:
        ...
        if isinstance(self.vqvae_model, torch.nn.parallel.DistributedDataParallel):
            recon = self.vqvae_model.module.decode_samples(embedding_indices=resampled_latent.to(self.device).long()).cpu().numpy()
        else:
    ...
    def resample(self, transformer_network, pet_in, ct_in, conditioning, rev_ordering, ind_sequencing):
        ...
        for i in range(zs_in.shape[-1] - 1):
            if mask[:, i].max() == 0:
                ...
                number_resampled += 1
                logits = transformer_network(sampled[:, :i + 1], ct_in, conditioning)[:, i, :]
        ...
        if latent_shape is None:
            ...
            latent_map = np.load('/nfs/home/apatel/normal_data/results/vqvae_ne128_dim32_PET_bottle_mse_adv/baseline_vqvae/testing_codes/014_broken_large_quantization_0.npy')
            latent_map = latent_map.astype(np.int16)
        ...
        upsampled_mask = upsampled_mask.repeat(8, axis=-1).repeat(8, axis=-2)
        upsampled_mask = np.expand_dims(upsampled_mask, 1)

        logits = transformer_network(sampled[:, 1:], ct_in, conditioning)[:, -1, :]
        probs_ = F.softmax(logits, dim=1)
...
class VQVAETransformerKDEZscoreInferer(TransformerConditionalInferer):
    def __init__(self, device: Any, threshold: float, embedding_shape: Any, vqvae_net:VQVAEBase,
        ...
        TransformerConditionalInferer.__init__(self, device=device, threshold=threshold, embedding_shape=embedding_shape, vqvae_net=vqvae_net, clip_encoding=clip_encoding)
    ...
    def __call__(self, inputs: torch.Tensor, network: VQVAEBase, *args: Any, **kwargs: Any):
        ...
        if len(inputs) == 4:
            ...
            input_pet, input_ct, conditioning, token_mask, original_pet = inputs 
            original_pet = original_pet.detach().cpu().numpy()
        #print(original_pet.shape)
        ...
        for i in range(self.num_passes_sampling):
            if i == 0:
                resampled_latents, resample_mask = self.resample(network, input_pet, input_ct, conditioning, revert_ordering, index_sequence)
            else:
                resampled_latent, _ = self.resample(network, input_pet, input_ct, conditioning, revert_ordering, index_sequence)
                resampled_latents = torch.cat((resampled_latents, resampled_latent), 0)
